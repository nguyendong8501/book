...
//		User user = userService.getUserById(loginname);
	...
	public String codeView(Model model) throws Exception {
		...
		if (noSendEmail == null) {
			...
			Mail mail = new Mail();
			mail.setMailFrom("booklib0101@gmail.com");
			mail.setMailTo(userForgot.getEmail());
			mail.setMailSubject("Quên Mật khẩu");
			mail.setMailContent("Mã đặt lại mật khẩu: " + code);
		...
		model.addAttribute("forgot", "Quên mật khẩu");
	...
	public String codeHandel(@ModelAttribute("code_input") int code_input, Model model) throws Exception {
		...
		if (code == code_input) {
			...
			session.setAttribute("error_code", "Mã không chính xác!");
			return "redirect:/code";
		}
